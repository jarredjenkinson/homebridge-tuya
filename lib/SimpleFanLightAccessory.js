...
class SimpleFanLightAccessory extends BaseAccessory {
    ...
    _registerCharacteristics(dps) {
        ...
        this.useColorTemperature = this._coerceBoolean(this.device.context.useColorTemperature, false);
        this.invertColorTemperature = this._coerceBoolean(this.device.context.invertColorTemperature, false);
        this.scaleColorTemperature = parseInt(this.device.context.scaleWhiteColor) || 255;
        ...
        if (this.useLight) {
            ...
            if (this.useColorTemperature) {
                const initialColorTemp = this.invertColorTemperature
                    ? this.scaleColorTemperature - dps[this.dpColorTemperature]
                ...
                characteristicColorTemperature = serviceLightbulb.getCharacteristic(Characteristic.ColorTemperature)
                    ...
                    .updateValue(this.convertColorTemperatureFromTuyaToHomeKit(initialColorTemp))
        ...
        this.device.on('change', (changes, state) => {
            ...
            if (changes.hasOwnProperty(this.dpBrightness) && characteristicBrightness && this.convertBrightnessFromHomeKitToTuya(characteristicBrightness.value) !== changes[this.dpBrightness])
                characteristicBrightness.updateValue(this.convertBrightnessFromTuyaToHomeKit(changes[this.dpBrightness]));

            if (changes.hasOwnProperty(this.dpColorTemperature) && characteristicColorTemperature) {
                const tuyaValue = this.invertColorTemperature
                    ? this.scaleColorTemperature - changes[this.dpColorTemperature]
                    : changes[this.dpColorTemperature];
                const homeKitValue = this.convertColorTemperatureFromTuyaToHomeKit(tuyaValue);
                if (characteristicColorTemperature.value !== homeKitValue)
                    characteristicColorTemperature.updateValue(homeKitValue);
            }
    ...
    getColorTemperature(callback) {
        this.getState(this.dpColorTemperature, (err, dp) => {
            if (err) return callback(err);
            const tuyaValue = this.invertColorTemperature ? this.scaleColorTemperature - dp : dp;
            callback(null, this.convertColorTemperatureFromTuyaToHomeKit(tuyaValue));
        });
    }

    setColorTemperature(value, callback) {
        if (value === 0) return callback(null, true);
        let tuyaValue = this.convertColorTemperatureFromHomeKitToTuya(value);
        if (this.invertColorTemperature) tuyaValue = this.scaleColorTemperature - tuyaValue;
        return this.setState(this.dpColorTemperature, tuyaValue, callback);
    }
}
