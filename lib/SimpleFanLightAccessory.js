...
class SimpleFanLightAccessory extends BaseAccessory {
    ...
    _registerCharacteristics(dps) {
        ...
        this.device.on('change', (changes, state) => {
            ...
            if (changes.hasOwnProperty(this.dpColorTemperature) && characteristicColorTemperature && this.convertColorTemperatureFromHomeKitToTuya(characteristicColorTemperature.value) !== changes[this.dpColorTemperature])
    ...
    getColorTemperature(callback) {
        this.getState(this.dpColorTemperature, (err, dp) => {
            if (err) return callback(err);
            callback(null, this.convertColorTemperatureFromTuyaToHomeKit(dp));
        });
    }

    setColorTemperature(value, callback) {
        if (value === 0) return callback(null, true);
        return this.setState(this.dpColorTemperature, this.convertColorTemperatureFromHomeKitToTuya(value), callback);
    }
}
